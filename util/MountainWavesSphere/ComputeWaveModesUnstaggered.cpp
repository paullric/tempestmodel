...
void GenerateEvolutionMatrix(
	...
	int nMatrixSize = 5 * param.nPhiElements;

	matM.Initialize(nMatrixSize, nMatrixSize);
	matB.Initialize(nMatrixSize, nMatrixSize);

	// Generate differentiation matrix
	DataMatrix<double> matDiff;
	matDiff.Initialize(param.nPhiElements, param.nPhiElements);
	...
	for (int j = 0; j < param.nPhiElements; j++) {
		...
		matM[ixV][ixU] = - dFr2 * (2.0 + dInvRo) * dSinPhi * dCosPhi;
		matM[ixP][ixU] = 1.0;

		// V evolution equations
		matM[ixU][ixV] = dFr2 * (2.0 + dInvRo) * dSinPhi * dCosPhi;
		matM[ixV][ixV] = - dK2 * dFr2;
		matM[ixP][ixV] = - dFr2 * (1.0 + dInvRo) * dSinPhi * dCosPhi;
		matM[ixR][ixV] = dFr2 * (1.0 + dInvRo) * dSinPhi * dCosPhi;

		for (int i = 0; i < param.nPhiElements; i++) {
			matM[5*i+2][ixV] += matDiff[j][i];
		}
		...
		matM[ixV][ixP] =
			- dFr2 * (1.0 + dInvRo) * dSinPhi * dCosPhi - dTanPhi;

		for (int i = 0; i < param.nPhiElements; i++) {
			matM[5*i+1][ixP] += matDiff[j][i];
		}
		...
		matM[ixV][ixR] = dFr2 * (1.0 + dInvRo) * dSinPhi * dCosPhi;
		...
		for (it = mapSortedRows.begin(); it != mapSortedRows.end(); it++) {
			...
			if (dLambdaI != 0.0) {
				...
				if (dMR > 0.0) {
					...
					for (int j = 0; j < param.nPhiElements; j++) {
						dUI  [j] = matVR[i+1][5*j  ];
						dVI  [j] = matVR[i+1][5*j+1];
						dPI  [j] = matVR[i+1][5*j+2];
						dWI  [j] = matVR[i+1][5*j+3];
						dRhoI[j] = matVR[i+1][5*j+4];
					}
