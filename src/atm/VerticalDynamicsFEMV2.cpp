///////////////////////////////////////////////////////////////////////////////
///
///	\file    VerticalDynamicsFEMV2.cpp
///	\author  Paul Ullrich
...
VerticalDynamicsFEMV2::VerticalDynamicsFEMV2(
	Model & model,
...
void VerticalDynamicsFEMV2::Initialize() {
	...
	SNESSetFunction(
		m_snes,
		m_vecR,
		VerticalDynamicsFEMV2_FormFunction,
		(void*)(this));
	...
	AnnounceStartBlock("Configuring VerticalDynamicsFEMV2");
...
void VerticalDynamicsFEMV2::StepImplicitTermsExplicitly(
	int iDataInitial,
	int iDataUpdate,
	const Time & time,
	double dDeltaT
...
*/
		...
		for (int j = box.GetBInteriorBegin(); j < box.GetBInteriorEnd(); j++) {
			...
			for (int k = 0; k < nRElements; k++) {
				m_dStateNode[RIx][k] = dataInitialNode[RIx][i][j][k];
			}
			...
			//////////////////////////////////////////////////////////////
			// Explicit vertical advection of horizontal velocity
			// (vertical curl terms)
			...
			for (int k = 0; k < nRElements; k++) {
				...
				dDeltaU /= (m_dStateNode[RIx][k] * dElementAreaNodeNode[i][j][k]);
				dDeltaV /= (m_dStateNode[RIx][k] * dElementAreaNodeNode[i][j][k]);

				dataUpdateNode[UIx][i][j][k] += dDeltaT * dDeltaU;
				dataUpdateNode[VIx][i][j][k] += dDeltaT * dDeltaV;
			}
...
void VerticalDynamicsFEMV2::BootstrapJacobian() {
...
void VerticalDynamicsFEMV2::StepImplicit(
	int iDataInitial,
	int iDataUpdate,
	const Time & time,
	double dDeltaT
...
void VerticalDynamicsFEMV2::SetupReferenceColumn(
	GridPatch * pPatch,
	int iA,
	int iB,
	const DataArray4D<double> & dataRefNode,
...
void VerticalDynamicsFEMV2::PrepareColumn(
	const double * dX
) {
	// Indices of EquationSet variables
	const int UIx = 0;
...
void VerticalDynamicsFEMV2::BuildF(
	const double * dX,
	double * dF
) {
	// Indices of EquationSet variables
...
void VerticalDynamicsFEMV2::BuildJacobianF_Diffusion(
	const double * dX,
	double * dDG
) {
	// Indices of EquationSet variables
...
void VerticalDynamicsFEMV2::BuildJacobianF_LOR_RhoTheta_Pi(
	const double * dX,
	double * dDG
) {
	// Indices of EquationSet variables
...
void VerticalDynamicsFEMV2::BuildJacobianF(
	const double * dX,
	double * dDG
) {
...
void VerticalDynamicsFEMV2::Evaluate(
	const double * dX,
	double * dF
) {
	// Prepare the column
...
void VerticalDynamicsFEMV2::UpdateColumnTracers(
	double dDeltaT,
	const DataArray4D<double> & dataInitialNode,
...
void VerticalDynamicsFEMV2::FilterNegativeTracers(
	int iDataUpdate
) {
...
#ifdef USE_JFNK_PETSC
PetscErrorCode VerticalDynamicsFEMV2_FormFunction(
	SNES snes,
	Vec x,
	Vec f,
	void * pDyn
	...
	((VerticalDynamicsFEMV2*)(pDyn))->Evaluate(dX, dF);
