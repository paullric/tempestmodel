...
// IMPLEMENTS ARK(2,3,1) FROM WELLER (Giraldo) ET AL. 2013 Fig. 2
...
void TimestepSchemeARK232::Step(
	...
	m_du2fCombo[1] = m_dExpCf[1][0] / m_dExpCf[0][0] - 
                         m_dImpCf[1][0] / m_dImpCf[0][1];
	m_du2fCombo[2] = m_dImpCf[1][0] / m_dImpCf[0][1];
        m_du2fCombo[3] = 0.0;
        m_du2fCombo[4] = 0.0;
        m_du2fCombo[5] = 0.0;
	...
	m_du3fCombo[1] = m_dExpCf[2][0] / m_dExpCf[0][0] - 
                         m_dImpCf[2][0] / m_dImpCf[0][1];
	m_du3fCombo[2] = m_dImpCf[2][0] / m_dImpCf[0][1];
	m_du3fCombo[3] = m_dExpCf[2][1] / m_dExpCf[1][1] - 
                         m_dImpCf[2][1] / m_dImpCf[1][2];
	m_du3fCombo[4] = m_dImpCf[2][1] / m_dImpCf[1][2];
	...
	pGrid->PostProcessSubstage(1, DataType_Tracers);
		...
		2, 2, time, m_dExpCf[0][0] * dDeltaT);
        pGrid->PostProcessSubstage(2, DataType_State);
        pGrid->PostProcessSubstage(2, DataType_Tracers);

	// Compute u1 into index 2 (NO IMPLICIT SOLVE NEEDED HERE)
	//pGrid->CopyData(1, 2, DataType_State);
        //pGrid->CopyData(1, 2, DataType_State);        
        //pVerticalDynamics->StepImplicit(
	//	2, 2, time, m_dImpCf[0][1] * dDeltaT);
        //pGrid->PostProcessSubstage(2, DataType_State);
	//pGrid->PostProcessSubstage(2, DataType_Tracers);
