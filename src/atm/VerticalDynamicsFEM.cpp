...
/*
	...
	dB.Initialize(m_nVerticalOrder+1);

	DataMatrix<double> dVan;
	dVan.Initialize(m_nVerticalOrder+1, m_nVerticalOrder+1);

	// Left value of reconstruction function is 1
	double dSign = 1.0;
	for (int i = m_nVerticalOrder; i >= 0; i--) {
		dVan[i][0] = dSign;
		dSign *= (-1.0);
	}
	dB[0] = 1.0;
	...
	dCoeff.Initialize(m_nVerticalOrder+1);
	...
	for (int n = 0; n < nType; n++) {
		...
		for (int i = 0; i < m_nVerticalOrder - n; i++) {
			dCoeff[i] =
				static_cast<double>(m_nVerticalOrder - n - i) * dCoeff[i];
		}
		for (int i = m_nVerticalOrder - n; i <= m_nVerticalOrder; i++) {
			dCoeff[i] = 0.0;
		}
	}

	// Polynomial is perpendicular to all spaces of polynomials up to P_{type-2}
	for (int n = 0; n < m_nVerticalOrder - nType; n++) {
	for (int m = 0; m <= m_nVerticalOrder; m++) {
		int nSumCoeff = (m_nVerticalOrder - m + n);
		if ((nSumCoeff % 2) == 0) {
			dVan[m][nType+1+n] = 2.0 / static_cast<double>(nSumCoeff + 1.0);
		}
	}
	}
	...
	DataVector<int> iPIV;
	iPIV.Initialize(m_nVerticalOrder+1);
	...
	for (int i = 0; i < m_nVerticalOrder; i++) {
		dB[m_nVerticalOrder-i] =
			static_cast<double>(i+1) * dB[m_nVerticalOrder-i-1];
	}
	dB[0] = 0.0;

	// Compute derivatives of reconstruction polynomial on nodes for the
	// reference element [0,1]
	m_dDiffReconsPolyNode.Initialize(m_nVerticalOrder);
	for (int n = 0; n < m_nVerticalOrder; n++) {
		double dX = 1.0;
		for (int i = 0; i < m_nVerticalOrder; i++) {
			m_dDiffReconsPolyNode[n] += dB[m_nVerticalOrder-i] * dX;
			dX *= (2.0 * dG[n] - 1.0);
		}
		m_dDiffReconsPolyNode[n] *= 2.0 / dElementDeltaXi;
	}

	// Compute derivatives of reconstruction polynomial on interfaces for the
	// reference element [0,1]
	m_dDiffReconsPolyREdge.Initialize(m_nVerticalOrder+1);
	for (int n = 0; n <= m_nVerticalOrder; n++) {
		double dX = 1.0;
		for (int i = 0; i < m_nVerticalOrder; i++) {
			m_dDiffReconsPolyREdge[n] += dB[m_nVerticalOrder-i] * dX;
			dX *= (2.0 * dGL[n] - 1.0);
		}
