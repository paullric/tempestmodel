...
// psuedo-variable for global attributes
...
NcDim* NcFile::get_dim( NcToken name ) const {
	int dimid;
	int status = NcError::set_err(nc_inq_dimid(the_id, name, &dimid));
...
NcVar* NcFile::get_var( NcToken name ) const {
	int varid;
	int status = NcError::set_err(nc_inq_varid(the_id, name, &varid));
	if(status != NC_NOERR) return NULL;
...
NcDim* NcFile::rec_dim( ) const {
	...
	int recdim;
	int status = NcError::set_err( nc_inq_unlimdim(the_id, &recdim) );
	if(status != NC_NOERR)
		return NULL;
...
NcVar* NcFile::add_var(
	NcToken name, NcType type, // scalar to 5D var
	const NcDim* dim0,
	const NcDim* dim1,
	const NcDim* dim2,
	const NcDim* dim3,
...
NcBool NcFile::sync( void ) {
	...
	int status = NcError::set_err(nc_sync(the_id));
	if (status != NC_NOERR)
		return FALSE;
...
NcBool NcFile::close( void ) {
	...
	int status = NcError::set_err( nc_close(old_id) );
	return (status == NC_NOERR);
}

NcBool NcFile::abort( void ) {
	int status = NcError::set_err( nc_abort(the_id) );
	return (status == NC_NOERR);
}
...
NcBool NcFile::define_mode( void ) {
	...
	int status = NcError::set_err( nc_redef(the_id) );
	if (status != NC_NOERR)
		return FALSE;
...
NcFile::NcFile(
	...
	switch (fmode) {
		...
		case Replace:
			...
			status = NcError::set_err(
				nc__create(path, mode, initialsize, bufrsizeptr, &the_id));
			if(status != NC_NOERR) {
				NcError::set_err(status);
				the_id =  -1;
			}
...
NcDim* NcVar::get_dim( int i ) const {
	...
	if(status != NC_NOERR || i < 0 || i >= ndim) {
	  return NULL;
	}
	return the_file->get_dim(dims[i]);
}

// edge lengths (dimension sizes)
...
// handles variable and global atts
...
NcBool NcVar::put(
	...
	int status = nc_put_vara_schar (the_file->id(), the_id, start, count, vals);
	return NcError::set_err(status) == NC_NOERR;
}
...
NcBool NcVar::put(
	...
	int status = nc_put_vara_text (the_file->id(), the_id, start, count, vals);
	return NcError::set_err(status) == NC_NOERR;
}
