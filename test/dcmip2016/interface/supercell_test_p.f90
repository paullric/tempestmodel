...
!
!  SUBROUTINE supercell_sample_p(
!    lon,lat,p,z,u,v,t,thetav,ps,rho,q)
!
...
!=======================================================================
  INTEGER(4), PARAMETER ::            &
       np         = 30         ,      & ! number of vertical levels in init
       nphi       = 20                  ! number of meridional points in init

  REAL(8), PARAMETER ::               &
       p0         = 100000.0d0      , & ! lower sample pressure
       p1         = 100.0d0             ! upper sample pressure
...
!----------------------------------------------------------------------- 
  ...
  REAL(8), DIMENSION(np)      :: zcoord
  REAL(8), DIMENSION(nphi,np) :: thetavyz
  REAL(8), DIMENSION(nphi,np) :: exneryz
  REAL(8), DIMENSION(np)      :: qveq
...
!=======================================================================
  SUBROUTINE supercell_init_p() &
    ...
    REAL(8), DIMENSION(np, np) :: ddp, intp
    ...
    REAL(8), DIMENSION(np, np) :: ddzbak
    REAL(8), DIMENSION(np, np) :: svdzu, svdzvt
    REAL(8), DIMENSION(np)     :: svdzs
    REAL(8), DIMENSION(5*np)   :: zwork

    ! Buffer data for calculation of SVD
    INTEGER(4) :: lwork, info

    ! Sampled values of ueq**2 and d/dz(ueq**2)
    REAL(8), DIMENSION(nphi, np) :: ueq2, dueq2

    ! Buffer matrices for iteration
    REAL(8), DIMENSION(nphi, np) :: phicoordmat, dptheta, rhs, irhs
  
    ! Buffer for sampled potential temperature at equator
    REAL(8), DIMENSION(np) :: thetaeq

    ! Buffer for computed equatorial Exner pressure and relative humidity
    REAL(8), DIMENSION(np) :: exnereq, H
    ...
    do k = 1, np
      pcoord(k) = - cos(dble(k-1) * pi / dble(np-1))
      pcoord(k) = p1 + 0.5d0*(p2-p1)*(pcoord(k)+1.0d0)
    end do
...
!-----------------------------------------------------------------------
  SUBROUTINE supercell_test(lon,lat,p,z,u,v,t,thetav,ps,rho,q,pert) &
